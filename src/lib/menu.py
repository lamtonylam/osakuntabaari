...
def create_menu_calendar_timed(menu):
    ...
    for date, food in menu.items():
        ...
        event.begin = EET.localize(datetime_object.replace(hour=11, minute=0))
        is_friday = datetime_object.weekday() == 4
        if is_friday:
            event.end = EET.localize(datetime_object.replace(hour=14, minute=30))
        else:
            event.end = EET.localize(datetime_object.replace(hour=15, minute=0))
        # Event description
        event.description = f"{date} \n{food}"
